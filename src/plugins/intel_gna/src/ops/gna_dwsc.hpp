...
int64_t calculate_num_spatial(const ov::intel_gna::op::GNADwsc* op,
...
void update_and_validate_attributes(ov::intel_gna::op::GNADwsc* op);

template <class T>
bool resolve_auto_pad_for_shape(const ov::intel_gna::op::GNADwsc* op,
                                ...
                                const int64_t& num_non_spatial_filter_dims);
template <class T>
void shape_infer(const ov::intel_gna::op::GNADwsc* op,
...
/// \brief DWSC with NHWC layout
///
class GNADwsc : public ov::op::Op {
public:
    OPENVINO_OP("GNADwsc", "intel_gna", ov::op::Op);

    /// \brief Constructs a DWSC operation.
    GNADwsc() = default;
    /// \brief Constructs a DWSC operation.
    ///
    ...
    ///
    GNADwsc(const ov::Output<ov::Node>& data_batch,
...
private:
    friend int64_t internal::calculate_num_spatial(const ov::intel_gna::op::GNADwsc* op,
                                                   const ov::PartialShape& input_shape,
                                                   const ov::PartialShape& filters_shape,
                                                   const int64_t& num_non_spatial_data_dims,
                                                   const int64_t& num_non_spatial_filter_dims);

    friend void internal::update_and_validate_attributes(ov::intel_gna::op::GNADwsc* op);

    template <class T>
    friend bool internal::resolve_auto_pad_for_shape(const ov::intel_gna::op::GNADwsc* op,
                                                     ov::CoordinateDiff& pads_begin,
                                                     ov::CoordinateDiff& pads_end,
                                                     const std::vector<T>& input_shapes,
                                                     const int64_t& num_non_spatial_data_dims,
                                                     const int64_t& num_non_spatial_filter_dims);
    template <class T>
    friend void internal::shape_infer(const ov::intel_gna::op::GNADwsc* op,
                                      const ov::CoordinateDiff& pads_begin,
                                      const ov::CoordinateDiff& pads_end,
                                      const std::vector<T>& input_shapes,
                                      std::vector<T>& output_shapes);
