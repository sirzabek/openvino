...
protected:
    void SetUp() override {
        ...
        auto eltwise = std::make_shared<ngraph::opset1::Add>(matmul, const_eltwise);
...
class ConvertMatmulToFcTransposedInputsPass : public testing::WithParamInterface<ConvertMatmulToFcPassParams >,
                              public LayerTestsUtils::LayerTestsCommon {
public:
    static std::string getTestCaseName(testing::TestParamInfo<ConvertMatmulToFcPassParams> obj) {
        InferenceEngine::Precision netPrecision;
...
protected:
    void SetUp() override {
        ...
        auto params = ngraph::builder::makeParams(ngPrc, {{ 1, inputShape[1][0] * inputShape[1][1] }});
        auto reshape1 = std::make_shared<ngraph::opset1::Reshape>(params[0],
            ngraph::builder::makeConstant<uint64_t>(ngraph::element::i64, {2}, inputShape[1]),
            false);
        std::vector<float> weights = CommonTestUtils::generate_float_numbers(inputShape[0][0] * inputShape[0][1], -0.1f, 0.1f);
        auto const_mult2 = ngraph::builder::makeConstant<float>(ngPrc, inputShape[0], weights);

        auto matmul = std::make_shared<ngraph::opset1::MatMul>(const_mult2, reshape1, true, true);

        auto out_shape = matmul->get_output_shape(0);
        auto reshape2 = std::make_shared<ngraph::opset1::Reshape>(matmul,
            ngraph::builder::makeConstant<uint64_t>(ngraph::element::i64, {2}, {1, out_shape[0] * out_shape[1]}),
            false);

        function = std::make_shared<ngraph::Function>(reshape2, params, "ConvertMatmulToFcTransposedInputs");
    }
};
...
TEST_P(ConvertMatmulToFcTransposedInputsPass, CompareWithRefImpl) {
    Run();
}
...
const std::vector<std::vector<std::vector<size_t>>> input_shapes_transposed_inputs = {
        {{64, 8}, {8, 64}},
        {{64, 80}, {1, 64}},
        {{2, 16}, {8, 2}}
};
...
INSTANTIATE_TEST_SUITE_P(smoke_convert_matmul_to_fc, ConvertMatmulToFcTransposedInputsPass,
                        ::testing::Combine(
                                ::testing::ValuesIn(input_shapes_transposed_inputs),
                                ::testing::ValuesIn(netPrecisions),
                                ::testing::Values(CommonTestUtils::DEVICE_GNA),
                                ::testing::ValuesIn(configs)),
                        ConvertMatmulToFcTransposedInputsPass::getTestCaseName);
} // namespace LayerTestsDefinitions
