...
static bool GetVerifiedTransposeData(const std::shared_ptr<opset9::Transpose> transpose, TransposeData& transpose_data) {
    ...
    auto order_const = std::dynamic_pointer_cast<opset9::Constant>(transpose->input_value(1).get_node_shared_ptr());
...
static bool DecomposeTransposeType1(const std::shared_ptr<opset9::Transpose> transpose,
    ...
    if ((H_new != transpose_data.H || W_new != transpose_data.W) &&
        ...
        auto reshape = std::make_shared<opset9::Reshape>(transpose_data.input,
            opset9::Constant::create(element::i32, Shape{2}, {H_new, W_new}), false);
        auto transpose_new = std::make_shared<opset9::Transpose>(reshape,
            opset9::Constant::create(element::i32, Shape{2}, {1, 0}));
        reshape = std::make_shared<opset9::Reshape>(transpose_new,
        ...
        if (feasible) {
            ...
            for (size_t i = 0; i < combined_factors.size(); i++) {
                auto reshape = std::make_shared<opset9::Reshape>(upstream[0],
                    opset9::Constant::create(element::i32, Shape{2}, {H_new * W_new / combined_factors[i], combined_factors[i]}), false);
                auto transpose = std::make_shared<opset9::Transpose>(reshape,
                    opset9::Constant::create(element::i32, Shape{2}, {1, 0}));
...
static bool DecomposeTransposeType2(const std::shared_ptr<opset9::Transpose> transpose,
                                      const TransposeData& transpose_data) {
    if (transpose_data.H <= 8 && transpose_data.W % GNALimitations::transposeMaxMinDim == 0 &&
        ...
        for (size_t c = 0; c < transpose_data.C; c++) {
            ...
            auto transpose_part = std::make_shared<opset9::Transpose>(reshape,
                opset9::Constant::create(element::i32, Shape{2}, {1, 0}));
            chunks.push_back(transpose_part);
        }

        auto concat = std::make_shared<opset9::Concat>(chunks, 0);
        auto transpose_final = std::make_shared<opset9::Transpose>(concat,
            opset9::Constant::create(element::i32, Shape{2}, {1, 0}));
        auto reshape = (transpose_data.input_size == 3) ? std::make_shared<opset9::Reshape>(transpose_final,
            ...
            : std::make_shared<opset9::Reshape>(transpose_final,
                opset9::Constant::create(element::i32, Shape{4}, {transpose_data.N, transpose_data.H, transpose_data.C, transpose_data.W}), false);
        ngraph::replace_node(transpose, reshape);
        reshape->set_friendly_name(transpose_data.name);
        return true;
    }
...
static std::shared_ptr<Node> GetTransposeOrder(const TransposeData& transpose_data) {
    ...
    if (transpose_data.input_size == 2) {
        transpose_const = op::Constant::create(element::i32, Shape{2}, {1, 0});
    } else if (transpose_data.input_size == 3) {
        transpose_const = op::Constant::create(element::i32, Shape{3}, {0, 2, 1});
    } else {
        transpose_const = op::Constant::create(element::i32, Shape{4}, {0, 1, 3, 2});
    }
    return transpose_const;
}

static bool DecomposeTransposeType3(const std::shared_ptr<opset9::Transpose> transpose,
                                    const TransposeData& transpose_data) {
    if (transpose_data.order == std::vector<int32_t>{0, 1, 3, 2} ||
        ...
        for (size_t c = 0; c < transpose_data.C; c++) {
            ...
            if (transpose_data.H <= 8) {
                auto new_transpose = std::make_shared<opset9::Transpose>(chunks[c], transpose_const);
                ...
                for (size_t i = 0; i < H_div_8; i++) {
                    auto new_transpose = std::make_shared<opset9::Transpose>(rowblock->output(i), transpose_const);
                ...
                for (size_t i = 0; i < H_div_8; i++) {
                    for (size_t j = 0; j < W_div_8; j++) {
                        auto new_transpose = std::make_shared<opset9::Transpose>(subblock[i][j], transpose_const);
                ...
                for (size_t j = 0; j < W_div_8; j++) {
                    auto new_transpose = std::make_shared<opset9::Transpose>(colblock[j], transpose_const);
        ...
        if (transpose_data.C > 1) {
            size_t axis = transpose_data.input_size - 3;
            auto concat = std::make_shared<opset9::Concat>(transpose_parts, axis);
...
static bool Convert(std::shared_ptr<Node> transpose_node) {
    const auto transpose = std::dynamic_pointer_cast<opset9::Transpose>(transpose_node);
...
static std::function<bool(Output<Node>)> verify_transpose() {
    return [=](Output<Node>& output) -> bool {
        ...
        if (GNALimitations::IsTransposeSupported(input_shape))
            return false;
...
DecomposeTranspose::DecomposeTranspose() {
    ...
    auto transpose_order = pattern::wrap_type<opset9::Constant>();
    auto transpose =
        pattern::wrap_type<opset9::Transpose>({pattern::any_input(), transpose_order}, verify_transpose());
