...
int64_t calculate_num_spatial(const GNADwsc* op,
...
void update_and_validate_attributes(GNADwsc* op) {
...
inline bool dynamic_check(const int64_t& num_spatial) {
    OPENVINO_ASSERT(num_spatial != -1,
                    "DWSC shape inference doesn't have enough information for static shape calculation");
    return true;
}
...
template <class T>
bool resolve_auto_pad_for_shape(const GNADwsc* op,
...
// TODO: search where that function is used in openvino
template <class T>
void shape_infer(const GNADwsc* op,
    ...
    *(output_shape.rbegin()) = filters_shape[0];
    ...
    NODE_VALIDATION_CHECK(
        op,
        n_data_channel.compatible(n_filter_channel),
...
GNADwsc::GNADwsc(const ov::Output<Node>& data_batch,
...
bool GNADwsc::visit_attributes(ov::AttributeVisitor& visitor) {
...
void GNADwsc::validate_and_infer_types() {
...
std::shared_ptr<ov::Node> GNADwsc::clone_with_new_inputs(const ov::OutputVector& new_args) const {
    if (new_args.size() == 2) {
        return std::make_shared<GNADwsc>(new_args.at(0),
                                                new_args.at(1),
                                                m_strides,
                                                m_pads_begin,
                                                m_pads_end,
    ...
    OPENVINO_THROW("Unsupported number of arguments for GNADwsc operation");
}
}  // namespace op
}  // namespace intel_gna
}  // namespace ov